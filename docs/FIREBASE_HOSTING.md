...
Step-by-step guide to deploy `gymbuddy-web` to Firebase Hosting.
...
From the **gymbuddy** project root (the folder that contains `gymbuddy-web`):
...
| **What do you want to use as your public directory?**            | `gymbuddy-web/dist`                                       |
| **Configure as a single-page app?**                              | Yes                                                       |
| **Set up automatic builds with GitHub?**                         | No (unless you want CI)                                   |
| **File gymbuddy-web/dist/index.html already exists. Overwrite?** | No                                                        |
...
From the **gymbuddy** project root:
...
**Note:** The app will call `http://localhost:8000/api/v1` by default. For production, create `gymbuddy-web/.env.production` with your API URL before building:
...
- Confirm the build output is in `gymbuddy-web/dist` and `firebase.json` points to it.
